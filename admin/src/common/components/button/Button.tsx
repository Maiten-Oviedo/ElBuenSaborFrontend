...
type ButtonProps = {
  ...
  disabled?: boolean
  className?: string
  icon?: React.ReactNode // opcion para incluir iconos ej:iconRight={<FiArrowRight size={18} />}
}
...
const Button: React.FC<ButtonProps> = ({
  ...
  let variantClasses = ''
  if (variant === 'primary') {
    variantClasses = 'bg-[#B85607] hover:bg-[#773703]'
  } else if (variant === 'secondary') {
    variantClasses = 'bg-white text-[#B85607] hover:text-black '
  }
  ...
  if (href) {
    return (
      <Link href={href} {...props} className={allClasses}>
        {content}
      </Link>
    )
  }
